...
package com.nari.rt21dms.qrcodelib.zxing.view;
...
import com.nari.rt21dms.qrcodelib.R;
...
public final class ViewfinderView extends View {
    ...
    private static long ANIMATION_DELAY = 20L;//改进扫描线的速度
    ...
    public  static  int  translationOffset;//扫描线的平移距离
    ...
    public ViewfinderView(Context context, AttributeSet attrs) {
        ...
        showPossiblePoint = typedArray.getBoolean(R.styleable.qr_ViewfinderView_qr_showPossiblePoint, true);
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (frame == null) {
            ...
            int width = 480;
            int height = 480;
            int leftOffset = (screenWidth - width) ;
            int topOffset = (screenHeight - height);
...
//            paint.setColor(frameColor);
    ...
    private void drawScanner(Canvas canvas, Rect frame) {
        if (showPossiblePoint) {
            ...
            canvas.drawRect(frame.left + 3, frame.top, frame.right - 3, frame.top + 3, paint);//改进扫描线的高度
            translateY += 5f;
            if (translateY >= translationOffset) {//改进控制扫描线的平移距离
                translateY = 5f;
            }
        }
    }
