...
import com.nari.rt21dms.scan.BuildConfig;
...
public class BamUIUtils {
    ...
    public static void froBig_ToSmall(View view) {
        try {
            float tzStart = 0;
            Object viewTag = view.getTag(R.string.view_tag_key);
            if (android.os.Build.VERSION.SDK_INT >= 21) {
                tzStart = view.getTranslationZ();
                if (viewTag == null || false == viewTag instanceof Float) {
                    view.setTag(R.string.view_tag_key, tzStart);
                }
            }
